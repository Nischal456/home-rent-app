...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const bill = await UtilityBill.findById(billId).populate('tenantId', 'fullName');
    if (!bill) {
    ...
    if (process.env.JWT_SECRET && token) {
      ...
      const adminMsg = `${tenantName} has paid Rs ${amount.toLocaleString()} towards their utility bill. Remaining balance: Rs ${newRemainingAmount.toLocaleString()}.`;
      const tenantMsg = `You have successfully paid Rs ${amount.toLocaleString()} towards your utility bill. Remaining balance: Rs ${newRemainingAmount.toLocaleString()}.`;

      const isPartial = updatedBill?.status === 'PARTIALLY_PAID';
      const titleAdmin = isPartial ? 'Partial Utility Payment Received' : 'Utility Payment Received';
      const titleTenant = isPartial ? 'Partial Utility Payment Confirmed' : 'Utility Payment Confirmed';
    ...
    console.error("Error processing utility payment:", errorMessage);

...
export function AddRoomForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const form = useForm<RoomFormValues>({
    ...
    defaultValues: { roomNumber: '', floor: '', rentAmount: 0 },
  });
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <FormField control={form.control} name="rentAmount" render={({ field }) => (
          <FormItem><FormLabel>Monthly Rent (Rs)</FormLabel><FormControl><Input type="number" placeholder="25000" {...field} /></FormControl><FormMessage /></FormItem>
        )} />

...
import { AddRentBillForm } from "@/app/dashboard/rent-bills/add-rent-bill-form";
    ...
    const handleToggleThreePhase = async () => {
        ...
        try {
            const res = await fetch(`/api/admin/tenants/${tenantId}`, {
                ...
                body: JSON.stringify({ hasThreePhaseMeter: !tenant.hasThreePhaseMeter }),
            });
            const data = await res.json();
            if (!res.ok || !data.success) throw new Error(data.message || 'Failed to update settings.');
            toast.success(tenant.hasThreePhaseMeter ? 'Three Phase Meter disabled.' : 'Three Phase Meter enabled!');
            mutate();
        } catch (err) {
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] pb-24 relative overflow-hidden selection:bg-[#0B2863]/20">
                        ...
                        <div className="w-full sm:max-w-md mx-auto p-6 overflow-y-auto max-h-[85vh]">
                            ...
                            <div className="space-y-1 mt-6 px-1">
                                ...
                                {selectedBill.type === 'Utility' && (() => {
                                    const utilityBill = selectedBill as IUtilityBill;
                                    return (
                                        <div className="bg-slate-50/50 p-4 rounded-3xl mt-4 border border-slate-100">
                                            ...
                                            <DetailRow icon={<Zap size={14} className="text-orange-400" />} label="Elec. Units" value={utilityBill.electricity.unitsConsumed} />
                                            <DetailRow icon={<Banknote size={14} className="text-emerald-500" />} label="Elec. Amount" value={`Rs ${utilityBill.electricity.amount.toLocaleString()}`} />
                                            {utilityBill.threePhase && utilityBill.threePhase.amount > 0 && (
                                                <>
                                                    <DetailRow icon={<Zap size={14} className="text-yellow-600" />} label="Three Phase Units" value={utilityBill.threePhase.unitsConsumed} />
                                                    <DetailRow icon={<Banknote size={14} className="text-emerald-500" />} label="Three Phase Amount" value={`Rs ${utilityBill.threePhase.amount.toLocaleString()}`} />
                                                </>
                                            )}
                                            <DetailRow icon={<Droplets size={14} className="text-blue-400" />} label="Water Units" value={utilityBill.water.unitsConsumed} />
                                            <DetailRow icon={<Banknote size={14} className="text-emerald-500" />} label="Water Amount" value={`Rs ${utilityBill.water.amount.toLocaleString()}`} />
                                            ...
                                            <DetailRow icon={<Wrench size={14} className="text-slate-400" />} label="Service Charge" value={`Rs ${utilityBill.serviceCharge.toLocaleString()}`} />
                                            <DetailRow icon={<Shield size={14} className="text-slate-400" />} label="Security Charge" value={`Rs ${utilityBill.securityCharge.toLocaleString()}`} />
                                        </div>
                                    );
                                })()}
            ...
            <div className="container mx-auto p-4 md:p-8 space-y-10 relative z-10 pt-8 md:pt-10">
                ...
                {/* Header Ribbon */}
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        ...
                        <Button onClick={() => setIsAddUtilityDialogOpen(true)} variant="outline" className="flex-1 md:flex-initial rounded-2xl h-12 font-bold shadow-sm hover:bg-slate-50 border-slate-200 text-slate-700 bg-white">
                            Add Utility Bill
                        </Button>
                    </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.15 }} className="w-full">
                    <Card className="border border-yellow-200/60 shadow-[0_8px_30px_rgba(0,0,0,0.02)] rounded-[2.5rem] bg-gradient-to-r from-yellow-50/40 via-amber-50/20 to-white/70 backdrop-blur-xl p-6 flex flex-col md:flex-row justify-between items-center gap-4">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-[1.3rem] bg-amber-100 flex items-center justify-center text-amber-600 shadow-inner flex-shrink-0">
                                <Zap className="w-6 h-6" />
                            </div>
                            <div>
                                <h4 className="font-extrabold text-slate-800 text-lg tracking-tight">Three Phase Line Extra Meter</h4>
                                <p className="text-xs font-semibold text-slate-500 mt-0.5">Toggle this setting to enable three-phase meter inputs for utility bills.</p>
                            </div>
                        </div>
                        <Button 
                            disabled={isUpdatingToggle} 
                            onClick={handleToggleThreePhase}
                            className={cn(
                                "rounded-2xl px-6 h-12 font-bold transition-all active:scale-95 flex items-center gap-2 text-sm shadow-md",
                                tenant?.hasThreePhaseMeter 
                        ...
                        >
                            {isUpdatingToggle ? (
                                ...
                                <>Disable Three Phase Meter</>
                            ) : (
                                <>Enable Three Phase Meter</>
                            )}
                        </Button>
                    </Card>
                </motion.div>
            ...
            <Dialog open={isAddRentDialogOpen} onOpenChange={setIsAddRentDialogOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle>Add Rent Bill</DialogTitle>
                        <DialogDescription>Draft a rent bill statement receipt.</DialogDescription>
                    </DialogHeader>
                    <AddRentBillForm onSuccess={() => { setIsAddRentDialogOpen(false); mutate(); }} preselectedTenantId={tenantId} />
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={isAddUtilityDialogOpen} onOpenChange={setIsAddUtilityDialogOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>Enter meter readings and charges for this tenant.</DialogDescription>
                    </DialogHeader>

...
export function PaymentDialog({ isOpen, onClose, rentBillsDue, utilityBillsDue, totalDue }: PaymentDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      ...
      */}
      <DialogContent className="sm:max-w-md md:max-w-3xl p-0 border-0 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl bg-white overflow-hidden flex flex-col w-[95vw] max-h-[90dvh] [&>button]:hidden">
              ...
              <div className="space-y-3">
                ...
                {rentBillsDue.map(bill => (
                  <div key={bill._id.toString()} className="flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.02)] hover:border-blue-100 transition-colors">
                    ...
                    <div className="text-right flex flex-col items-end">
                      <span className="font-black text-[#0B2863] text-base md:text-lg">Rs {(bill.remainingAmount ?? bill.amount)?.toLocaleString('en-IN') ?? 'N/A'}</span>
                      {bill.status === 'PARTIALLY_PAID' && <span className="text-[10px] font-bold text-orange-500 uppercase tracking-wider mt-0.5">Remaining</span>}
                    </div>
                  </div>
                ))}
                ...
                {utilityBillsDue.map(bill => (
                  <div key={bill._id.toString()} className="flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.02)] hover:border-orange-100 transition-colors">
                    ...
                    <div className="text-right flex flex-col items-end">
                      <span className="font-black text-slate-900 text-base md:text-lg">Rs {(bill.remainingAmount ?? bill.totalAmount)?.toLocaleString('en-IN') ?? 'N/A'}</span>
                      {bill.status === 'PARTIALLY_PAID' && <span className="text-[10px] font-bold text-orange-500 uppercase tracking-wider mt-0.5">Remaining</span>}
                    </div>
                  </div>
                ))}

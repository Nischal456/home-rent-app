...
import { Loader2, PlusCircle, MoreHorizontal, FilePlus2, AlertCircle, Trash2, CheckCircle2, Zap, Share2, Printer, Search, Calendar, Hash, FileText, CircleUserRound, Receipt, Banknote, Droplets, Wrench, Shield } from 'lucide-react';
...
const UtilityBillCard = ({ bill, onAction, onShare, onPrint, onClick }: { bill: UtilityBillData; onAction: (action: 'pay' | 'delete', bill: UtilityBillData) => void; onShare: (bill: UtilityBillData) => void; onPrint: (bill: UtilityBillData) => void; onClick: () => void }) => {
    const tenant = bill.tenantId as IUser;
    const room = bill.roomId as IRoom;

    return (
        <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, transition: { duration: 0.2 } }} onClick={onClick} className="cursor-pointer">
            <Card className="overflow-hidden shadow-sm border-l-[3px] border-l-primary">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 p-3 pb-1">
                    <div className="flex-1 min-w-0 pr-2">
                        <CardTitle className="text-sm font-semibold truncate">{tenant?.fullName ?? 'Unknown'}</CardTitle>
                        <p className="text-[10px] text-muted-foreground font-medium truncate">Room: {room?.roomNumber ?? 'N/A'}</p>
                    </div>
                    <Badge variant={bill.status === 'PAID' ? 'default' : bill.status === 'PARTIALLY_PAID' ? 'outline' : 'destructive'} className={`text-[9px] h-4 px-1 ${bill.status === 'PARTIALLY_PAID' ? 'border-primary text-primary' : ''}`}>
                        {bill.status}
                    </Badge>
                </CardHeader>
                <CardContent className="p-3 pt-1 pb-2">
                    <div className="flex justify-between items-end mb-1">
                        <div>
                            <p className="text-[9px] text-muted-foreground uppercase tracking-wider mb-0.5">Total Bill</p>
                            <div className="text-lg font-bold text-primary leading-none">Rs {bill.totalAmount.toLocaleString()}</div>
                        </div>
                ...
                <CardFooter className="flex justify-between items-center bg-muted/30 p-2 px-3 border-t">
                    <p className="text-[9px] text-muted-foreground">Due: {bill.billDateBS}</p>
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild><Button variant="ghost" className="h-6 w-6 p-0 rounded-full"><MoreHorizontal className="h-3 w-3" /></Button></DropdownMenuTrigger>
                        <DropdownMenuContent align="end">
                            ...
                            {bill.status !== 'PAID' && <DropdownMenuItem onClick={() => onAction('pay', bill)}><CheckCircle2 className="mr-2 h-4 w-4" /> Record Payment</DropdownMenuItem>}
            ...
            <div className="space-y-4 pb-32">
                <AnimatePresence>
                    {filteredBills.map((bill) => (
                        <UtilityBillCard key={bill._id.toString()} bill={bill} onClick={() => setSelectedBill(bill)} onAction={(action, b) => setConfirmation({ action, bill: b })} onShare={handleShare} onPrint={printBill} />
                    ))}
                </AnimatePresence>
            </div>
        ) : (
            ...
            <AlertDialog open={!!confirmation && confirmation.action === 'delete'} onOpenChange={() => setConfirmation(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>This action will permanently delete this bill.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction className="bg-red-600 hover:bg-red-700" onClick={handleAction}>Continue</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
